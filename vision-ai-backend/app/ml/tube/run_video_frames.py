...
def filter_to_coil_cluster(ends, cluster_dist=350.0):
...
def main():
    ...
    try:
        while True:
            ...
            # 2c. Discard unattached false detections (wrist watch, table scratches) when tubings are present
            tubings = [m for c, m in inst if c == class_map[2]]
            if tubings:
                attached = [e for e in ends if e.get("tubing_id") is not None]
                if attached:
                    ends = attached
