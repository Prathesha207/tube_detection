...
Hand short-circuit
------------------
- MediaPipe Hands (default) runs on every frame. If a hand is detected, the
  whole duck pipeline is SKIPPED for that frame -> {"status":"HAND", ...}.
- Gate region: the detected tray if present; if no tray this frame, the drawn
  ROI (hand_roi.json); if neither exists, any hand anywhere skips.
- Occlusion guard: after anchor lock, if `occlusion_drop` or more ducks vanish
  at once (arm over the tray that MediaPipe missed), the frame is treated as
  occluded (status "HAND") for up to `occlusion_max_frames` frames.
- Works with BOTH mediapipe APIs: the legacy `mp.solutions.hands` and the newer
  Tasks API `HandLandmarker`, for builds where `mp.solutions` was removed. The
  backend is chosen automatically at startup (see _init_mediapipe_hands); the
  Tasks path needs a small `hand_landmarker.task` file, downloaded once and
  cached next to config.yaml.
...
class DuckAnalyzer:
    def __init__(self, config_path, expected_duck_count=None):
        ...
        self.cfg = cfg
        # remembered so the mediapipe Tasks model file can be cached next to it
        self._config_path = os.path.abspath(config_path)
        # Robustly resolve model_path across any working directory / caller
        configured_model = cfg.get("model_path", "best.pt")
        candidates = []
        if configured_model:
            if os.path.isabs(configured_model) and os.path.exists(configured_model):
                candidates.append(configured_model)
            else:
                cfg_dir = os.path.dirname(os.path.abspath(config_path))
                candidates.append(os.path.join(cfg_dir, configured_model))
                this_dir = os.path.dirname(os.path.abspath(__file__))
                ml_dir = os.path.dirname(this_dir)
                candidates.append(os.path.join(ml_dir, configured_model))
                candidates.append(os.path.join(this_dir, configured_model))
                candidates.append(os.path.join(os.getcwd(), configured_model))
        this_dir = os.path.dirname(os.path.abspath(__file__))
        ml_dir = os.path.dirname(this_dir)
        candidates.extend([
        ...
        self.model_path = resolved_model if resolved_model else configured_model
        ...
        self.missing_report_frames = int(cfg.get("missing_report_frames", 3))
        ...
        self._mp_hands = None
        # Tasks-API landmarker; stays None on builds that still have
        # mp.solutions, so the legacy path below is used unchanged.
        self._mp_tasks = None
        self._mp_image_cls = None
        self._mp_fmt = None
        self.hand_hold_frames = int(cfg.get("hand_hold_frames", 5))
        self._hand_hold = 0
        # occlusion guard: a sudden multi-duck drop = arm/hand over the tray
        self.occlusion_drop = int(cfg.get("occlusion_drop", 3))
        self.occlusion_max_frames = int(cfg.get("occlusion_max_frames", 90))
        self._occl_frames = 0

        # -------- hand ROI (draw once, then fixed) --------
        # Used as the hand gate on frames where NO tray is detected: a hand
        # only short-circuits the frame if it overlaps this rectangle. When a
        # tray IS detected, the tray box is the gate instead. The ROI is stored
        # as [x, y, w, h] in a small JSON file (roi_path) so it persists across
        # runs: draw it one time (see calibrate_roi / draw_roi.py). With no ROI
        # file and no tray, any hand anywhere skips the frame.
        self.roi_path = cfg.get("roi_path", "hand_roi.json")
        if self.roi_path and not os.path.isabs(self.roi_path):
            cfg_dir = os.path.dirname(os.path.abspath(config_path))
            this_dir = os.path.dirname(os.path.abspath(__file__))
            for r_cand in [
                os.path.join(cfg_dir, self.roi_path),
                os.path.join(this_dir, self.roi_path),
                os.path.join(os.path.dirname(this_dir), self.roi_path),
            ]:
                if os.path.exists(r_cand):
                    self.roi_path = r_cand
                    break
        ...
        # ---- bottle suppression (drops ducks/toys detected on a bottle) ----
        # A standard pretrained COCO model (class 39 = bottle) runs alongside
        # the duck model. Any duck/toy detection whose CENTER lies on a
        # detected bottle is dropped before anchor/matching/counting, so a
        # bottle cap can never be counted or locked as a duck. No training or
        # labelling needed; yolo11n.pt downloads automatically the first time.
        self.bottle_suppress = bool(cfg.get("bottle_suppress", False))
        self.bottle_model = None
        self.bottle_conf = float(cfg.get("bottle_conf", 0.3))
        self.bottle_pad_frac = float(cfg.get("bottle_pad_frac", 0.05))
        if self.bottle_suppress:
            self.bottle_model = YOLO(cfg.get("bottle_model_path", "yolo11n.pt"))
            try:
                self.bottle_model.to(self._device_str)
            except Exception as e:
                print(f"[BOTTLE] [WARN] could not move bottle model: {e}")
            print("[BOTTLE] bottle suppression ON")
        ...
        try:
            try:
                from .embedder import DeepEmbedder      # installed as a package / relative
            except (ImportError, ValueError):
                try:
                    from app.ml.debug.embedder import DeepEmbedder  # absolute package import
                except ImportError:
                    from embedder import DeepEmbedder        # run as a flat script / on sys.path
        ...
        if self.hand_backend == "mediapipe":
            self._init_mediapipe_hands(cfg)
    ...
    # ------------------------------------------------------------------ #
    #  MediaPipe hand backend init (works across mediapipe versions)      #
    # ------------------------------------------------------------------ #
    def _init_mediapipe_hands(self, cfg):
        """Set up hand detection with whichever mediapipe API this build has.

        Older mediapipe exposes the legacy `mp.solutions.hands.Hands`. Some
        builds ship WITHOUT `mp.solutions` entirely, so `mp.solutions.hands`
        raises AttributeError -- those provide the Tasks API instead
        (`mediapipe.tasks.python.vision.HandLandmarker`), which needs a small
        `.task` model file, downloaded once and cached beside config.yaml.

        Both paths end up giving `_hand_boxes` the same thing: normalized
        landmarks it turns into pixel boxes. Nothing else in the class changes.
        """
        try:
        ...
        mp_ver = getattr(mp, "__version__", "?")

        # --- preferred: legacy solutions API (unchanged behaviour) ---
        if hasattr(mp, "solutions") and hasattr(mp.solutions, "hands"):
            ...
            print(f"[HAND] mediapipe {mp_ver} -> legacy solutions.hands API")
            return
        ...
        try:
            ...
            raise ImportError(
                f"This mediapipe build ({mp_ver}) has neither "
                f"mp.solutions.hands nor the Tasks API ({e}). Install a build "
                f"that has one of them, or set hand_backend: yolo in "
                f"config.yaml.")

        task_path = cfg.get("hand_task_path", "hand_landmarker.task")
        if not os.path.isabs(task_path):
            task_path = os.path.join(os.path.dirname(self._config_path), task_path)
        if not os.path.exists(task_path):
            url = ("https://storage.googleapis.com/mediapipe-models/"
                   "hand_landmarker/hand_landmarker/float16/1/"
                   "hand_landmarker.task")
            print(f"[HAND] hand_landmarker.task not found -> downloading to "
                  f"{task_path}")
            try:
                ...
                raise RuntimeError(
                    f"could not download the hand landmarker model ({e}). "
                    f"Download it manually from {url}, place it at "
                    f"{task_path}, and run again -- or set hand_backend: yolo "
                    f"in config.yaml.")
        ...
        self._mp_tasks = mp_vision.HandLandmarker.create_from_options(opts)
        self._mp_image_cls = mp.Image
        self._mp_fmt = mp.ImageFormat.SRGB
        print(f"[HAND] mediapipe {mp_ver} -> Tasks API HandLandmarker "
              f"({task_path})")
    ...
    def _bottle_boxes(self, frame):
        """Padded (x1,y1,x2,y2) boxes of bottles found by the pretrained COCO
        model (class 39). Empty list when bottle suppression is off."""
        if self.bottle_model is None:
            return []
        r = self.bottle_model.predict(source=frame, conf=self.bottle_conf,
                                      classes=[39], imgsz=self.imgsz,
                                      device=self._device_str, verbose=False)[0]
        out = []
        if r.boxes is not None and len(r.boxes) > 0:
            for x1, y1, x2, y2 in r.boxes.xyxy.cpu().numpy():
                px = (x2 - x1) * self.bottle_pad_frac
                py = (y2 - y1) * self.bottle_pad_frac
                out.append((x1 - px, y1 - py, x2 + px, y2 + py))
        return out
        ...
        try:
            ...
            if self.roi_path and os.path.exists(self.roi_path):
                with open(self.roi_path, "r") as f:
                    d = json.load(f)
                r = d.get("roi", d) if isinstance(d, dict) else d  # accept {"roi":[...]} or a bare [...]
                if r and len(r) == 4:
                    self.roi = tuple(int(v) for v in r)
                    print(f"[ROI] loaded hand ROI {self.roi} from {self.roi_path}")
            else:
                print(f"[ROI] no ROI file at {self.roi_path} -> "
                      f"no-tray frames fall back to any-hand-skips")
        except Exception as e:
    ...
    def _hand_boxes(self, frame):
        ...
        frame (empty if none). Works for the mediapipe Tasks API, the legacy
        mediapipe solutions API, and the yolo backend."""
        boxes = []
        h, w = frame.shape[:2]

        # --- mediapipe Tasks API (builds without mp.solutions) ---
        if self._mp_tasks is not None:
            ...
            # timestamp; frame_idx * 33 stands in for ~30 fps input.
            res = self._mp_tasks.detect_for_video(mp_img, int(self.frame_idx * 33))
            for lms in (getattr(res, "hand_landmarks", None) or []):
    ...
    def process_frame(self, frame):
        ...
        # ============================================================== #
        #  DETECT FIRST, THEN GATE THE HAND SHORT-CIRCUIT                 #
        # ============================================================== #
        # Order:
        #   1. track()            -> all detections incl. the tray
        #   2. pick the tray box  -> _pick_tray_box (coasts if briefly missing)
        #   3. hand gate          -> tray present: hand must overlap the tray
        #                            no tray:      hand must overlap the drawn ROI
        #                            no tray, no ROI: any hand anywhere skips
        ...
        # ---- drop any detection whose centre lies on a detected bottle ----
        if self.bottle_model is not None:
            bottles = self._bottle_boxes(frame)
            if bottles:
                dets = [(xyxy, c, cf, tid) for (xyxy, c, cf, tid) in dets
                        if not any(self._center_in_box(xyxy, b) for b in bottles)]

        # tray box for BOTH the hand gate and the object gate below
        tray_box = self._pick_tray_box(dets)

        # ---- hand short-circuit, gated by the tray (or ROI if no tray) ----
        ...
        if hand_boxes:
            if tray_box is not None:
                ...
                # no tray this frame -> use the drawn ROI if there is one
                if self.roi is not None:
                    hand_now = any(self._rects_overlap(b, self.roi) for b in hand_boxes)
                else:
                    hand_now = True
        else:
            hand_now = False
        ...
        if hand_now or self._hand_hold > 0:
            ...
            # draw the gate region (tray, else ROI) and the hand banner
            if tray_box is not None:
        ...
        # ---- occlusion guard (catches hands MediaPipe missed) ----
        # After lock, if several ducks vanish at once it is almost always an
        # arm/hand covering the tray, not a real loss. Treat those frames as
        # HAND for up to occlusion_max_frames; if the drop lasts longer, the
        # normal pipeline runs and reports it as a real anomaly.
        if self.anchor_locked and self.occlusion_drop > 0:
            big_drop = detected <= self.expected - self.occlusion_drop
            self._occl_frames = self._occl_frames + 1 if big_drop else 0
            if big_drop and self._occl_frames <= self.occlusion_max_frames:
                annotated = frame.copy()
                if tray_box is not None:
                    tx1, ty1, tx2, ty2 = [int(v) for v in tray_box]
                    cv2.rectangle(annotated, (tx1, ty1), (tx2, ty2), ORANGE, 2)
                    ...
                    rx, ry, rw, rh = self.roi
                    cv2.rectangle(annotated, (rx, ry), (rx + rw, ry + rh), ORANGE, 2)
                draw_banner(annotated, "OCCLUDED", ORANGE)
                return self._finish(
                    annotated, status="HAND", detected=detected, fps=fps,
                    hand_detected=True,
                    missing_ids=[], added_ids=[], other_ids=[],
                    detections=[], thumbnails=[], reasons=[])
        ...
        for xyxy, tid, crop in det_list:
            ...
            if did is not None and did in self.display_info and did not in used_slots:
        ...
        free_slots = [d for d in self.display_info if d not in used_slots]
        ...
        for did in range(1, self.num_anchor + 1):
            if did in present_display:
                ...
                if self.display_info[did]["gone_frames"] >= self.missing_report_frames:
                    missing_ids.append(did)
        ...
        for did, (xyxy, cf) in present_display.items():
            x1, y1, x2, y2 = [int(v) for v in xyxy]
            # An excess duck is any duck beyond expected count or beyond anchor count
            is_excess = (self.expected is not None and did > self.expected) or (did > self.num_anchor)
            if is_excess:
                excess_ids.append(int(did))
            this_color = RED if is_anomaly else GREEN
            detections.append({"bbox": [x1, y1, x2, y2], "id": int(did),
                               ...
                               "excess": bool(is_excess),
                               "is_anomaly": bool(is_excess or is_anomaly)})
            lbl = f"#{did} EXCESS {int(cf * 100)}%" if is_excess else f"#{did} {int(cf * 100)}%"
            draw_box(annotated, xyxy, lbl, this_color)

        for xyxy, cf in unbound:
            x1, y1, x2, y2 = [int(v) for v in xyxy]
            # An unbound duck (strict_count mode: a detection that matched no
            # slot) is an extra/unexpected duck -> always red, even if the
            # frame is otherwise NORMAL, so the offending duck stands out.
            detections.append({"bbox": [x1, y1, x2, y2], "id": -1,
                               "species": "duck", "confidence": round(float(cf), 4),
                               "status": "unbound", "excess": True,
                               "is_anomaly": True})
        ...
        if self.debug:
            print(f"[DBG f{self.frame_idx}] status={status} detected={detected} "
                  f"present={sorted(present_display.keys())} excess={excess_ids} pending={len(pend_meta)} "
